...
@test "Print LLVM installation information" {
    ...
    echo "sudo apt --purge remove 'libomp5' 'libc++*dev' 'libc++*' 'python3-lldb-*' 'llvm-libunwind1' libunwind-*-dev' 'libclc-*' 'libclc-*dev' 'libmlir-*'"
    echo
...
@test "Test libclc package presence" {
    if ! test -f "/usr/lib/clc/amdgcn-amd-amdhsa.bc"; then
...
@test "Test pthread_cancel with libunwind compatibility" {
    ...
    skip "test is wrong, see https://bugs.debian.org/1118641"
